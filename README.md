...
- **Prompt snippets** — Keeps the half-written messages a user retypes every day as ordinary Markdown files in `~/.claude/prompts`, one per file, and offers them as a second `/` group beside the Claude command catalog. Picking one drops its text into the composer instead of sending it, so the draft stays editable — a snippet is a message the user finishes, not a command that runs. The session menu saves the current draft as a new snippet, offering its opening line as the file name; an existing name is never overwritten. Editing, renaming, and deleting happen in the user's own editor, and a file added there shows up in the menu without a reload.
