...
- **Claude permission modes in the access selector** — Extends DSH's access selector with the Claude Code modes its sandbox knob cannot express: `Ask Every Time` (ask before every tool, edits included), `Auto Approve` (a model classifier answers the prompts), and `Allowlist Only` (run what the Claude settings allowlist permits, deny the rest without prompting), alongside the upstream `Read Only`, `Workspace Write`, and `Full access`. A preset claims its mode by name through the plugin's `permissionModes` config, so a deployment can add or rename presets without touching this package; a preset that claims nothing folds its sandbox mode as before.
