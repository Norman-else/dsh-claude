...
export function installClaudeEventVocabulary() {
...
export function redactText(value, maxChars = MAX_DETAIL_CHARS) {
    return boundText(value
        ...
        .replace(SECRET_ASSIGNMENT, `$1${REDACTED}`), maxChars);
}
...
export function normalizeActivity(activity) {
    ...
    if (activity.phase !== undefined)
        normalized.phase = activity.phase;
    if (activity.toolUseId !== undefined)
        normalized.toolUseId = redactText(activity.toolUseId, 256);
    if (activity.toolName !== undefined)
        normalized.toolName = redactText(activity.toolName, 256);
    if (activity.title !== undefined)
        normalized.title = redactText(activity.title, MAX_SUMMARY_CHARS);
    if (activity.summary !== undefined) {
        normalized.summary = redactText(typeof activity.summary === 'string' ? activity.summary : safeDetail(activity.summary) ?? '', MAX_SUMMARY_CHARS);
    }
    const detail = safeDetail(activity.detail);
    if (detail !== undefined)
        normalized.detail = detail;
    if (activity.isError !== undefined)
        normalized.isError = activity.isError;
    if (activity.usage !== undefined)
        normalized.usage = { ...activity.usage };
    return normalized;
}
export function currentClaudeActivityCursor(events) {
    let turn = 0;
    let step = 0;
    let nextOrdinal = 0;
...
export async function appendClaudeActivity(agent, cursor, activity) {
...
export async function appendClaudeSessionBinding(agent, binding) {
    const payload = {
        claudeSessionId: binding.claudeSessionId,
        sdkVersion: binding.sdkVersion ?? SDK_VERSION,
        cwd: binding.cwd,
    };
    if (binding.cliVersion !== undefined)
        payload.cliVersion = binding.cliVersion;
    await agent.session.append(CLAUDE_SESSION_BOUND_EVENT, payload);
    return payload;
}
export function latestClaudeSessionBinding(events) {
