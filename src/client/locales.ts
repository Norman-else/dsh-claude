export const zh = {
  ...
  defaultPermissionModeEffect: '会话在你用 composer 里的权限选择器改过之前所运行的 Claude Code 权限模式。DSH 的访问预设仍决定沙箱：沙箱承载不了的默认值（比如只读沙箱下的自动模式）会退回沙箱自身对应的模式；当前模型不支持自动模式时退回默认模式。每回合读取，未手动选过模式的会话从下一回合起生效。',
  renderer: 'AI 输出渲染器',
...
export const en = {
  ...
  defaultPermissionModeEffect: "The Claude Code permission mode a session runs under until you change it with the composer's own selector. DSH's access preset still decides the sandbox: a default the sandbox cannot carry (auto under read-only) yields to the sandbox's own mode, and auto yields to Default on a model that cannot run it. Read every turn, so a session that never chose picks a change up on its next turn.",
