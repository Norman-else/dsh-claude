export const zh = {
  ...
  rendererEffect: '插件渲染器沿用本插件自带的转录视图：交错的正文、成组的工具卡片与活动行。DSH 原生渲染器改由 DSH 自身绘制：正文作为普通助手文本块，思考作为推理块，Claude 的顶层工具会镜像成原生工具卡片。切换后需重启 DSH 才会生效；已经产生的回合仍按记录时的渲染器显示。',
